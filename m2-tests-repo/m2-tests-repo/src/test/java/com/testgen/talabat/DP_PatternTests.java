...
/**
 * TC379–TC425 — Design Pattern checks for Milestone 2 (Talabat theme).
 *
...
public class DP_PatternTests extends TestBase {
    ...
    void tc379_refundStrategyInterfaceExists() throws Exception {
        ...
        assertFalse(src.isEmpty(),
            ...
            "Per spec §3.2, checkout-service must declare: interface RefundStrategy { … calculateRefund(…); }");
    ...
    @Test
    @DisplayName("TC382 — DP-1 Strategy: FullRefundWithDeliveryStrategy audit trail")
    void tc382_fullRefundAuditTrail() throws Exception {
        ...
        // Seed COMPLETED payment (today) — amount=150, deliveryFee=20
        ...
        HttpResponse<String> r = httpPostAuth(
                "/api/payments/" + pid + "/refund-with-fee-handling", body, tok);
        assert2xx(r, "TC382 FullRefund POST");
        JsonNode j = parseNode(r.body());
        double refundAmt = _refundAmtFromBody(j);
        assertEquals(150.0, refundAmt, 0.5,
            "TC382: refundAmount must be 150 (full); got=" + refundAmt + " body=" + r.body());
    ...
    @Test
    @DisplayName("TC383 — DP-1 Strategy: FoodOnlyRefundStrategy audit trail")
    void tc383_foodOnlyRefundAuditTrail() throws Exception {
        BASE_URL = checkoutServiceUrl;
        if (mongo == null) throw new AssertionError("TC383: MongoDB required.");
        // amount=200, deliveryFee=30 → food portion = 170
        long pid = _S5F12Seed.seedPayment(jdbc, this, "COMPLETED", 200.0, 30.0,
                new java.sql.Timestamp(System.currentTimeMillis()));
        String tok = adminToken();
        String body = "{\"reason\":\"food_quality\",\"refundDeliveryFee\":false}";
        HttpResponse<String> r = httpPostAuth(
                "/api/payments/" + pid + "/refund-with-fee-handling", body, tok);
        assert2xx(r, "TC383 FoodOnly POST");
        JsonNode j = parseNode(r.body());
        double refundAmt = _refundAmtFromBody(j);
        assertEquals(170.0, refundAmt, 0.5,
            "TC383: refundAmount must be 170 (food only, no delivery fee); got=" + refundAmt);
        com.mongodb.client.MongoCollection<org.bson.Document> col =
                mongo.getCollection(s5AuditCollection());
        org.bson.Document latest = col
                .find(new org.bson.Document("paymentId", pid))
    ...
    @Test
    @DisplayName("TC384 — DP-1 Strategy: NoRefundStrategy 400 + audit")
    void tc384_noRefundAuditOnDenial() throws Exception {
        BASE_URL = checkoutServiceUrl;
        if (mongo == null) throw new AssertionError("TC384: MongoDB required.");
        // 48 hours ago — past the 24-hour refund window
        ...
        assertEquals(400, r.statusCode(),
            "TC384: expired payment must return 400; got=" + r.statusCode() + " body=" + r.body());
        ...
        if (latest == null)
            latest = col.find().sort(new org.bson.Document("_id", -1)).first();
        assertNotNull(latest,
            "TC384: REFUND_DENIED event must be logged BEFORE the 400 is thrown (per spec §3.2 step e); " +
            "no document found for paymentId=" + pid);
    ...
    @Test
    @DisplayName("TC385 — DP-1 Strategy: refund service has no if-else on refundDeliveryFee")
    void tc385_noIfElseOnRefundDeliveryFeeInService() throws Exception {
        BASE_URL = checkoutServiceUrl;
        for (java.nio.file.Path p : allJavaFiles()) {
            ...
            if (!content.contains("refundDeliveryFee")) continue;
            assertFalse(content.contains("if (refundDeliveryFee") ||
                        content.contains("if(refundDeliveryFee") ||
                        content.contains("refundDeliveryFee ?") ||
                        content.contains("refundDeliveryFee?"),
                "TC385: " + fname + " branches on refundDeliveryFee directly. " +
    ...
    void tc389_registerTriggersAuthEvent() throws Exception {
        ...
        // Find registration event for this user
    ...
    void tc392_unregisterMethodExists() throws Exception {
        BASE_URL = userServiceUrl;
        // Behavioural unregister test requires instantiating student classes — impossible from
        // an external test-runner. We verify the structural contract instead: the subject class
        // must expose a removeObserver / unregisterObserver / detach method, proving the chain
        // is load-bearing rather than a decorative wrapper around a direct Mongo call.
    ...
    void tc394_atLeastThreeConcreteHandlers() throws Exception {
        BASE_URL = userServiceUrl;
        // Count files that extend or implement AuthHandler (excluding AuthHandler itself)
    ...
    void tc397_deletedUserToken401() throws Exception {
        ...
        // Fresh user has no orders/payments — DELETE should succeed; fall back to DEACTIVATE
        try {
    ...
    void tc400_filterDelegatesToChainHead() throws Exception {
        ...
        // Filter must invoke the chain head — look for any handle() call pattern
        ...
        // Filter must NOT do inline token parsing — that belongs in SignatureValidationHandler
    ...
    void tc401_m2DashboardDtosHaveBuilder() throws Exception {
        ...
        for (String dto : dtos) {
            ...
            // Accept builder() in the class itself OR a companion <DTO>Builder class
            boolean hasBuilder =
                src.contains("builder()") ||
                src.contains("class Builder") ||
                src.contains("Builder<")  ||
                src.contains("new Builder(") ||
                !readClassSource(dto + "Builder").isEmpty();
    ...
    void tc402_m1DtosHaveBuilder() throws Exception {
        ...
        String[] dtos = {
            ...
            "TopCustomerDTO",          // S1-F6
            "UserProfileDTO",          // S1-F8
            "RestaurantRevenueDTO",    // S2-F3
            "TopRestaurantDTO",        // S2-F6
            "RestaurantMenuAlertDTO",  // S2-F9
            "OrderCostEstimateDTO",    // S3-F3
            "OrderAnalyticsDTO",       // S3-F6
            "OrderDetailsDTO",         // S3-F9
            "NearbyDeliveryDTO",       // S4-F3
            "DeliveryPerformanceDTO",  // S4-F8
            "DelayedDeliveryDTO",      // S4-F9
        ...
        for (String dto : dtos) {
            ...
            // Accept inner Builder, companion <DTO>Builder class, or builder() method
            boolean hasBuilder =
                src.contains("builder()") ||
                src.contains("class Builder") ||
                src.contains("Builder<")  ||
    ...
    void tc403_m2DashboardRegressionAfterBuilderRetrofit() throws Exception {
        ...
        // Create a restaurant via HTTP
        String phone = "+201" + nonce().substring(0, 9);
        String email = "tc403." + nonce() + "@test.com";
        String restBody = String.format(
            "{\"name\":\"TC403 Restaurant\",\"cuisineType\":\"ITALIAN\",\"phone\":\"%s\",\"email\":\"%s\"}",
            phone, email);
        HttpResponse<String> create = httpPostAuth("/api/restaurants", restBody, adminTok);
        assert2xx(create, "TC403 create restaurant");
        long rid = parseNode(create.body()).get("id").asLong();
        // JDBC insert 3 DELIVERED orders summing to 600
        String oTable = tableName("Order");
        for (int i = 0; i < 3; i++) {
            Map<String, Object> ov = new HashMap<>();
            ov.put(columnByField("Order", "user"),        1L);
            ov.put(columnByField("Order", "restaurant"),  rid);
            ov.put(columnByField("Order", "totalAmount"), 200.0);
            ov.put(columnByField("Order", "status"),      "DELIVERED");
            insertRowReturningId(oTable, ov);
        }
        HttpResponse<String> dash = httpGetAuth(
                "/api/restaurants/" + rid + "/dashboard", adminTok);
        assert2xx(dash, "TC403 GET dashboard");
        JsonNode j = parseNode(dash.body());
        assertTrue(j.has("totalOrders") || j.has("total_orders") || j.has("ordersCount"),
            "TC403: dashboard missing totalOrders after Builder retrofit; body=" + dash.body());
        assertTrue(j.has("totalRevenue") || j.has("total_revenue") || j.has("revenue"),
            "TC403: dashboard missing totalRevenue after Builder retrofit; body=" + dash.body());
    }
    ...
    void tc404_m1RetrofitBehaviorUnchanged() throws Exception {
        ...
        long uid = (long)(Number) u.get("id");
        String tok = (String) u.get("token");
        // Seed a restaurant + 3 DELIVERED orders
        String rTable = tableName("Restaurant");
        Map<String, Object> rOv = new HashMap<>();
        rOv.put(columnByField("Restaurant", "name"),        "TC404 Rest");
        rOv.put(columnByField("Restaurant", "cuisineType"), "ITALIAN");
        long rid = insertRowReturningId(rTable, rOv);
        String oTable = tableName("Order");
        for (int i = 0; i < 3; i++) {
            Map<String, Object> ov = new HashMap<>();
            ov.put(columnByField("Order", "user"),        uid);
            ov.put(columnByField("Order", "restaurant"),  rid);
            ov.put(columnByField("Order", "totalAmount"), 100.0);
            ov.put(columnByField("Order", "status"),      "DELIVERED");
            insertRowReturningId(oTable, ov);
        }
    ...
    void tc405_s2F8AndS3F8ExemptFromBuilder() throws Exception {
        BASE_URL = userServiceUrl;
        // S2-F8 toggles MenuItem availability → returns entity, not DTO → no Builder needed.
        // S3-F8 adds items to an Order → returns entity, not DTO → no Builder needed.
        // Check that entity classes (Restaurant, MenuItem, Order) themselves don't declare
        // builder() — if they do, students over-applied the Builder pattern to non-DTOs.
        for (String entityName : new String[]{"Restaurant", "MenuItem", "Order"}) {
            String src = readClassSource(entityName);
            if (src.isEmpty()) continue;
            // Entities must not carry a Builder inner class
            assertFalse(src.contains("static class Builder") ||
                        src.contains("class Builder<"),
                "TC405: " + entityName + " entity must NOT have a Builder inner class. " +
                "S2-F8 (toggle) and S3-F8 (addItems) return entities directly — Builder is " +
    ...
    void tc407_getInstanceIsPublicStatic() throws Exception {
        ...
        // Verify public + static modifiers co-appear with getInstance
    ...
    void tc411_jwtServiceUsesGetInstance() throws Exception {
        BASE_URL = userServiceUrl;
        // Structural: JwtService must call JwtConfigurationManager.getInstance()
    ...
    void tc413_fiveEventClassesImplementMongoEvent() throws Exception {
        BASE_URL = userServiceUrl;
        String[] events = {
            "AuthEvent", "RestaurantEvent", "OrderEvent", "DeliveryEvent", "PaymentAuditEvent"
        };
    ...
    void tc416_allFiveEventTypesDispatched() throws Exception {
        ...
        List<String> missing = new ArrayList<>();
        for (String t : new String[]{"AUTH", "RESTAURANT", "ORDER", "DELIVERY", "PAYMENT_AUDIT"}) {
    ...
    void tc420_adapterClassesPresent() throws Exception {
        ...
        if (!anySourceContains("class ElasticsearchHitAdapter"))
            missing.add("ElasticsearchHitAdapter (restaurant-service)");
        ...
        if (!anySourceContains("class CassandraRowAdapter"))
            missing.add("CassandraRowAdapter (delivery-service)");
        assertTrue(missing.isEmpty(),
    ...
    @Test
    @DisplayName("TC423 — DP-7 Adapter: ElasticsearchHitAdapter (restaurant-service)")
    ...
    void tc425_jpqlFeaturesExemptFromAdapter() throws Exception {
        ...
        // The grader must not false-positive on those features.
