...
public abstract class TestBase {
    ...
    private Object _defaultValueFor(java.util.Map<String, Object> col) {
        ...
        switch (javaType) {
            case "String":
                // email/phone are NOT NULL UNIQUE on entities like User and Restaurant
                // (per M1 spec). Without per-call uniqueness, the SECOND
                // buildKitchenSinkBody call in any test that creates 2+ entities
                // (TC37/38/39/41/42 etc.) hits a duplicate-key 400 in setup.
                if ("email".equalsIgnoreCase(fieldName)) {
                    return "default_" + nonce() + "@grader.testgen.io";
                }
                if ("phone".equalsIgnoreCase(fieldName)
                        || "phoneNumber".equalsIgnoreCase(fieldName)) {
                    String n = nonce();
                    return "+2010" + n.substring(0, Math.min(9, n.length()));
                }
    ...
    protected String s2CatalogFkColumn() {
        ...
        // First try: proper FK constraint in information_schema
        try {
            return jdbc.queryForObject(
        ...
        } catch (DataAccessException ignored) { /* fall through to name-based lookup */ }
        // Fallback: look for a column in the order table whose name contains the catalog
        // entity name or common aliases (restaurant_id, catalog_id, etc.)
        String catalogLc = catalogTable.toLowerCase();
        String s2EntityLc = s2CatalogEntity().toLowerCase();
        try {
            java.util.List<String> cols = jdbc.queryForList(
                "SELECT column_name FROM information_schema.columns "
              + "WHERE table_schema='public' AND table_name=? "
              + "ORDER BY ordinal_position",
                String.class, orderTable);
            for (String col : cols) {
                String lc = col.toLowerCase();
                if (lc.contains(catalogLc) || lc.contains(s2EntityLc) || lc.equals("restaurant_id")) {
                    return col;
                }
            }
        } catch (DataAccessException ignored) {}
    ...
    protected String firstTopLevelNonUserEntity() {
        return firstTopLevelNonUserEntityInfo()[0];
    }

    /**
     * Returns [entityClassName, baseUrl] for the first top-level non-User entity.
     * Prefers user-service entities (packageName contains ".user.") so TC06/TC07
     * can use userServiceUrl. Falls back to any service if user-service has none,
     * deriving the base URL from the entity's package name.
     */
    @SuppressWarnings("unchecked")
    protected String[] firstTopLevelNonUserEntityInfo() {
        java.util.List<java.util.Map<String, Object>> entities =
            (java.util.List<java.util.Map<String, Object>>) manifest().get("entities");
        String fallbackCls = null;
        if (entities != null) {
            // Pass 1: prefer user-service entities
            for (java.util.Map<String, Object> e : entities) {
                String cls = (String) e.get("className");
                if (cls == null || "User".equals(cls)) continue;
                String pkg = (String) e.getOrDefault("packageName", "");
                if (!pkg.contains(".user.")) continue;
                try {
                    String collPath = crudCollectionPath(cls);
                    if (collPath != null && !collPath.contains("{"))
                        return new String[]{cls, userServiceUrl};
                } catch (IllegalStateException ignored) {}
            }
            // Pass 2a: prefer catalog/restaurant-service entities (most stable CRUD)
            for (java.util.Map<String, Object> e : entities) {
                String cls = (String) e.get("className");
                if (cls == null || "User".equals(cls)) continue;
                String pkg = (String) e.getOrDefault("packageName", "");
                if (!pkg.contains(".restaurant.") && !pkg.contains(".catalog.") && !pkg.contains(".menu.")) continue;
                try {
                    String collPath = crudCollectionPath(cls);
                    if (collPath == null || collPath.contains("{")) continue;
                    return new String[]{cls, catalogServiceUrl};
                } catch (IllegalStateException ignored) {}
            }
            // Pass 2b: any top-level entity — derive URL from package
            for (java.util.Map<String, Object> e : entities) {
                String cls = (String) e.get("className");
                if (cls == null || "User".equals(cls)) continue;
                String pkg = (String) e.getOrDefault("packageName", "");
                try {
                    String collPath = crudCollectionPath(cls);
                    if (collPath == null || collPath.contains("{")) continue;
                    String url = _serviceUrlFromPackage(pkg);
                    return new String[]{cls, url};
                } catch (IllegalStateException ignored) {}
            }
        }
    ...
    private String _serviceUrlFromPackage(String pkg) {
        if (pkg == null) return userServiceUrl;
        if (pkg.contains(".catalog.") || pkg.contains(".restaurant.")) return catalogServiceUrl;
        if (pkg.contains(".order."))    return orderServiceUrl;
        if (pkg.contains(".delivery.")) return deliveryServiceUrl;
        if (pkg.contains(".checkout.") || pkg.contains(".payment.")) return checkoutServiceUrl;
    ...
    /** Per-spec id property name for a Neo4j label. Spec §6.3 uses
     *  &lt;lowerCase(labelHead)&gt;Id (e.g., User → userId, Restaurant → restaurantId).
     *  Tests must NOT assume the bare property "id" — that contradicts
     *  the spec and would only match a non-conforming student. */
    protected String neo4jIdProp(String label) {
        if (label == null || label.isEmpty()) return "id";
        return Character.toLowerCase(label.charAt(0)) + label.substring(1) + "Id";
    }

    /** Count nodes with a given label whose spec-correct id property equals id. */
    protected long neo4jNodeCount(String label, long id) {
        if (neo4j == null) return -1L;
        String prop = neo4jIdProp(label);
        java.util.List<java.util.Map<String, Object>> rows = neo4jExec(
            "MATCH (n:`" + label + "` {`" + prop + "`: $id}) RETURN count(n) AS c",
     ...
     *  labelled nodes (matched on each label's spec-correct id property). */
    protected boolean neo4jRelExists(String fromLabel, long fromId,
                                     String relType,
                                     String toLabel,   long toId) {
        if (neo4j == null) return false;
        String fProp = neo4jIdProp(fromLabel);
        String tProp = neo4jIdProp(toLabel);
        java.util.List<java.util.Map<String, Object>> rows = neo4jExec(
            "MATCH (a:`" + fromLabel + "` {`" + fProp + "`: $fromId})-[r:`" + relType
          + "`]->(b:`" + toLabel + "` {`" + tProp + "`: $toId}) RETURN count(r) AS c",
     ...
     *  relationship. Matches each endpoint by its spec-correct id property. */
    protected long neo4jRelOrderCount(String fromLabel, long fromId,
        ...
        java.util.List<java.util.Map<String, Object>> rows = neo4jExec(
            "MATCH (a:`" + fromLabel + "` {`" + fProp + "`: $fromId})-[r:`" + relType
          + "`]->(b:`" + toLabel + "` {`" + tProp + "`: $toId}) RETURN r.orderCount AS oc LIMIT 1",
            java.util.Map.of("fromId", fromId, "toId", toId));
    ...
    /** Read a string-typed relationship property. Endpoints matched by
     *  spec-correct id property (see {@link #neo4jIdProp}). */
    protected String neo4jRelStringProp(String fromLabel, long fromId,
        ...
        java.util.List<java.util.Map<String, Object>> rows = neo4jExec(
            "MATCH (a:`" + fromLabel + "` {`" + fProp + "`: $fromId})-[r:`" + relType
          + "`]->(b:`" + toLabel + "` {`" + tProp + "`: $toId}) RETURN r.`" + propName
          + "` AS v LIMIT 1",
            java.util.Map.of("fromId", fromId, "toId", toId));
    ...
    /** Spec (M2 §S5-F12 step (i)) persists refundAmount in the transactionDetails JSONB
     *  column, NOT at the response top level. The Payment entity serialization typically
     *  surfaces transactionDetails as a nested object. Read top-level first (some students
     *  also flatten), then fall back to the nested JSONB tree under common key names. */
    protected double _refundAmtFromBody(com.fasterxml.jackson.databind.JsonNode j) {
        if (j == null) return -1.0;
        if (j.has("refundAmount") && j.get("refundAmount").isNumber())
            return j.get("refundAmount").asDouble();
        if (j.has("refund_amount") && j.get("refund_amount").isNumber())
            return j.get("refund_amount").asDouble();
        for (String k : new String[]{"transactionDetails", "transaction_details"}) {
            com.fasterxml.jackson.databind.JsonNode td = j.get(k);
            if (td == null || td.isNull()) continue;
            // Spring serializes JSONB as either an object node or a string of JSON.
            if (td.isTextual()) {
                try {
                    td = new com.fasterxml.jackson.databind.ObjectMapper().readTree(td.asText());
                } catch (Exception ignored) { continue; }
            }
            if (td.has("refundAmount") && td.get("refundAmount").isNumber())
                return td.get("refundAmount").asDouble();
            if (td.has("refund_amount") && td.get("refund_amount").isNumber())
                return td.get("refund_amount").asDouble();
        }
        return -1.0;
    }
